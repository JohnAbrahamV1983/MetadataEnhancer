...
export default function AgenticSearchPage() {
  ...
  const { data: searchResults, isLoading, error } = useQuery({
    queryKey: ["/api/agentic-search", searchQuery],
    queryFn: async () => {
      if (!searchQuery) return null;
      const response = await fetch(`/api/agentic-search?q=${encodeURIComponent(searchQuery)}`);
      if (!response.ok) {
  ...
  const handleSearch = async () => {
    ...
    // The query will trigger automatically via useQuery
    setTimeout(() => setIsSearching(false), 500); // Brief loading state for UX
  };
